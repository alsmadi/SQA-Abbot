...
public class ComponentTester extends Robot {
    ...
    */ 
    ...
    // Bregis
    /** Directly set the given text to the given component.
     * Used for rapid repeat the same actions.
     * Also that method not depends on the keyboard layout 
     * unlike {@link #actionKeyString(Component, String)}
     */
    public void actionSetText(Component c, String string) {
        actionFocus(c);
        setText(string);
        waitForIdle();
    }
    ...
    // Bregis
    /** Send the string suitable today's date (in format ddMMyyyy) to the given component. */
    public void actionKeyTodaysDate(Component c) {
        actionFocus(c);
        actionKeyString(new SimpleDateFormat("ddMMyyyy").format(Calendar.getInstance().getTime()));
        waitForIdle();
    }
