...
public class PropertyCallTest extends ResolverFixture {

    public void testChangeType() throws Throwable {
        ComponentReference ref =
            new ComponentReference(getResolver(), Component.class,
                                   new HashMap());
        getResolver().addComponentReference(ref);
        PropertyCall step = new PCall(getResolver(), getName(),
                                      abbot.tester.
                                      ComponentTester.class.getName(),
                                      "assertFrameShowing",
                                      new String[] { "title" });
        assertEquals("Default class should be ComponentTester",
                     abbot.tester.ComponentTester.class.getName(),
                     step.getTargetClassName());
        step.setComponentID(ref.getID());
        assertEquals("Target class should have changed",
                     Component.class.getName(), step.getTargetClassName());
    }

    public void testMethodLookup() throws Throwable {
        String text = "Custom Field";
        // TODO need to check a class that *wasn't* loaded by the same class
        // loader as this test.  
    ...
    public void testStaticMethodCall() throws Throwable {
        PropertyCall step = new PCall(getResolver(), getName(),
                                               getClass().getName(),
                                               "staticCall", new String[0]);
        assertEquals("Wrong default class", getClass(), step.getTargetClass());
    }

    public PropertyCallTest(String name) { super(name); }
