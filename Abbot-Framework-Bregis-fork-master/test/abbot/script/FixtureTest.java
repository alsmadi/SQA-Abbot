...
public class FixtureTest extends TestCase {
    ...
    public void testLaunchWithExtantSameFixture() throws Throwable {
        ...
        assertFalse("Same fixture should not be launched twice", 
                    fixtureDup.wasRun);
        assertTrue("Script 1 should be considered launched", 
                   script.getUIContext().isLaunched());
        assertTrue("Script 2 should be considered launched", 
                   scriptDup.getUIContext().isLaunched());
    }
    ...
    public void testRunWithExtantSameFixture() throws Throwable {
        ...
        assertFalse("Fixture should not be run twice",
                    fixture.wasRun);
        assertTrue("Script should still be considered launched",
                   script.getUIContext().isLaunched());
        assertTrue("Script should still be considered launched",
                   scriptDup.getUIContext().isLaunched());
    }
    ...
    public void testLaunchWithExtantDifferentFixture() throws Throwable {
        ...
        assertFalse("First script should have terminated on launch of second", 
                    script.getUIContext().isLaunched());
        assertTrue("Second script should indicate launched", 
                   script2.getUIContext().isLaunched());
    ...
    public void testRunWithExtantDifferentFixture() throws Throwable {
        ...
        assertTrue("Second script should indicate launched",
                   script2.getUIContext().isLaunched());
        assertTrue("Second fixture should have run",
                   fixture2.wasRun);
    }
    
    public void testPreserveHierarchyDuringContext() throws Throwable {
        Hierarchy runnerHierarchy = runner.getHierarchy();
        assertTrue("Wrong runner default Hierarchy before run: " + runnerHierarchy, 
                   runnerHierarchy instanceof TestHierarchy);
        runner.run(script);
        ...
        assertEquals("New runner should get Hierarchy from fixture",
                     fixtureHierarchy, runner2.getHierarchy());
        assertEquals("New runner should set the new script's Hierarchy", 
                     fixtureHierarchy, scriptDup.getHierarchy());
    }

    public void testParseScriptWithFixture() throws Throwable {
        String SCRIPT = 
            "<AWTTestScript>"
            + "<fixture filename=\"" + fixture.getFilename() + "\"/>"
            + "</AWTTestScript>";
        script.load(ScriptTest.stringToReader(SCRIPT));
        assertEquals("Wrong number of steps", 1, script.steps().size());
        assertTrue("First step should be a script",
                   script.steps().get(0) instanceof Script);
        assertTrue("First step should be a fixture",
                   ((Script)script.steps().get(0)) instanceof Fixture);
    }
    ...
    private class TestFixture extends Fixture implements DynamicLoadingConstants {
        ...
        public TestFixture(Script parent, String id) {
            ...
            addStep(new Launch(this, id, DYNAMIC_CLASSNAME, "main", 
                               new String[] { "[]" }, DYNAMIC_CLASSPATH, false));
