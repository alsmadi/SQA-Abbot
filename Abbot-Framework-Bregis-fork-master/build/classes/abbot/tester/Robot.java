...
//    }
    ...
    */
    ...
    public void drag(Component src, int sx, int sy, int buttons) {
        ...
        if (Platform.isWindows() || Platform.isMacintosh()) {
            ...
            // Bregis:
            // if the source object is an instance of some list class,
            // src.getHeight() will return height of the whole list (not one list item),
            // so dy = AWTConstants.DRAG_THRESHOLD may withdraw the mouse pointer on other list item
            if (src instanceof javax.swing.JList)
                dy = 0;
    ...
    */
    ...
    // Bregis
    /** Directly sets the given string. */
    public void setText(String str) {
        Component focus = findFocusOwner();
        if (focus == null) {
            Log.warn("No component has focus, setText discarded",
                    Log.FULL_STACK);
            return;
        }
        try {
            ((JTextComponent) focus).setText(str);
        } catch (Exception e) {
            Log.warn("Component can not be cast to JTextComponent",
                    Log.FULL_STACK);
        }
    }
